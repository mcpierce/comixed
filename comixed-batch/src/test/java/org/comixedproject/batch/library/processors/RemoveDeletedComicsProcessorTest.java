...
class RemoveDeletedComicsProcessorTest {
  ...
  void process_withDeleteFile() {
    ...
    verify(fileAdaptor).deleteFile(file);
    verify(comicAdaptor).deleteMetadataFile(comic);
  }
  ...
  void process() {
    ...
    verify(comicService).deleteComic(comic);
    verify(fileAdaptor, never()).deleteFile(any());
  }
