...
class RemoveDeletedComicsChunkListenerTest {
  private static final long TEST_TOTAL_COMICS = 717L;
  private static final long TEST_REMAINING_COMICS = 129L;

  @InjectMocks private RemoveDeletedComicsChunkListener listener;
  ...
  public void setUp() throws PublishingException {
    ...
    when(jobInstance.getJobName()).thenReturn(ORGANIZE_LIBRARY_JOB);
    ...
    when(comicService.findComicsToPurgeCount()).thenReturn(TEST_REMAINING_COMICS);
  ...
  void afterChunk_publishingException() throws PublishingException {
    doThrow(PublishingException.class).when(publishBatchProcessDetailUpdateAction).publish(any());
    listener.beforeChunk(chunk);
